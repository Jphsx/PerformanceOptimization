...
// Tue Aug  6 14:43:25 2019 by ROOT version 6.12/07
// from TTree PhotonConversionsTree/PhotonConversionsTree
// found on file: datalinks/Run2018_110.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   testtree(std::string ifile, TTree *tree=0);
   virtual ~testtree();
...
#ifdef testtree_cxx
testtree::testtree(std::string ifile, TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(ifile.c_str());
      if (!f || !f->IsOpen()) {
         f = new TFile(ifile.c_str());
      }
      TDirectory * dir = (TDirectory*)f->Get((ifile+":/MyNtupleMaking").c_str());
