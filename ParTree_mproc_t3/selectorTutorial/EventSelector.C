...
//auto it = fParticles_fPosX.end();
//std::cout<<fParticles_fPosX.end()<<std::endl;
//	for(auto it = fParticles_fPosX.begin(); it!=fParticles_fPosX.end(); ++it){
		//std::cout<<*it<<" ";
		
//	}
//	std::cout<<std::endl;
//   for(int i=0; i<*fEventSize; i++){
//	for( auto it = fParticles_fPosX.begin(), auto it2 = fParticles_fMomentum.begin(); it != fParticles_fPosX.end(); it++, it2++){
	//if( fParticles_fMomentum[] > 40. ){
	//	if(*it2 > 40.){
		//hfPosX->Fill(fParticles_fPosX[i]);
	//	hfPosX->Fill(*it);
//	}
//}
int i =0;
 for(auto ipart = fParticles_fPosX.begin();  ipart != fParticles_fPosX.end(); ++ipart){
	if(fParticles_fMomentum[i] > 40.){
		hfPosX->Fill(*ipart);
	}
	i++;
}
